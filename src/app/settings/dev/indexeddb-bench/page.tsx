...
function buildUserStreamRows(rows: number) {
  const out = [];
  for (let i = 0; i < rows; i++) {
    const stream = [];
    const len = 8 + (i % 5);
...
function buildTagStreamRows(rows: number) {
  const out = [];
  for (let i = 0; i < rows; i++) {
    const stream = [];
    const len = 6 + (i % 4);
...
function buildUserStreamWorkloads({
  table,
  rows,
  config,
  flags,
...
function buildTagStreamWorkloads({
  table,
  rows,
  config,
}: {
    ...
    },
    {
      ...
      run: async () => {
        for (let i = 0; i < config.lookups; i++) {
          ...
          const matches = await table
            .filter((row: any) => Array.isArray(row.stream) && row.stream.some((tag: any) => tag?.label === target))
            .toArray();
          matches.slice(0, 100);
        }
      },

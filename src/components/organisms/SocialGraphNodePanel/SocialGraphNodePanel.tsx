...
export function SocialGraphNodePanel({
  ...
  const { isFollowing, isLoading: isFollowStatusLoading } = useIsFollowing(targetPubky);
    ...
    >
      <div className="flex items-center justify-between">
        <Typography size="sm" className="font-medium text-muted-foreground uppercase">
          {KIND_LABELS[node.kind === 'post' && node.is_reply ? 'reply' : node.kind]}
        </Typography>
        ...
        <>
          ...
          {proofUsers.length > 0 && (
            ...
            >
              ...
              />
              <Typography size="sm" className="text-muted-foreground">
                {`followed by ${proofUsers.length.toLocaleString('en-US')} you follow`}
              </Typography>
            </button>
          )}
      ...
      {node.kind === 'tag' && (
        <>
          <div className="flex items-center gap-2">
            <Tag name={node.label} />
            <Typography size="sm" className="text-muted-foreground">
              {`used ${node.count.toLocaleString('en-US')} ${node.count === 1 ? 'time' : 'times'}`}
            </Typography>
          </div>
          <div className="flex gap-2">
            {expandButton}

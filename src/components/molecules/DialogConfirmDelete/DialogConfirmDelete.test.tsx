...
describe('DialogConfirmDelete', () => {
  ...
  it('interpolates the collection name in delete confirmation copy', () => {
    function CollectionDeleteDialog() {
      const t = useTranslations('dialogs.deleteCollection');
      return (
        <DialogConfirmDelete
          open={true}
          onOpenChange={vi.fn()}
          onConfirm={vi.fn()}
          i18nNamespace="dialogs.deleteCollection"
          description={t('description', { name: 'Proof of Work' })}
        />
      );
    }

    render(<CollectionDeleteDialog />);
    ...
    expect(screen.getByTestId('dialog-description')).not.toHaveTextContent('{name}');
  });

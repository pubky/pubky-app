...
describe('usePassportAuth', () => {
  ...
  it('fails immediately when the popup is blocked', async () => {
  ...
  it('fails when the popup closes without a recorded success', async () => {
    const flow = createFlow();
    mockGetPassportAuthUrl.mockResolvedValue(flow);
    const onAttemptSettled = vi.fn();
    const hook = renderHook(() => usePassportAuth({ onAttemptSettled }));
  ...
  it('clears the timeout once the session is accepted so slow initialization cannot fail the attempt', async () => {
    ...
    expect(listeners).toHaveLength(0); // message listener torn down at the transition

    // Deadline passes while initialization is still running.
    await act(async () => {
  ...
  it('ends quietly when a newer auth flow supersedes it (controller cancels the flow)', async () => {
    const flow = createFlow();
    mockGetPassportAuthUrl.mockResolvedValue(flow);

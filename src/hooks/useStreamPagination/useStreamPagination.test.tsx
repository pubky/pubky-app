...
describe('useStreamPagination', () => {
  ...
  describe('Refresh', () => {
    ...
    it('keeps existing posts visible while a network refresh is pending', async () => {
      vi.mocked(StreamPostsController.getOrFetchStreamSlice).mockResolvedValueOnce({
        nextPageIds: ['existing-post'],
        nextCursor: 10,
        reachedEnd: false,
      });
      ...
      expect(result.current.loading).toBe(false);
      expect(result.current.postIds).toEqual(['existing-post']);

      refreshPage.resolve({ nextPageIds: ['fresh-post'], nextCursor: 20, reachedEnd: true });
      await act(async () => {
  ...
  describe('Stream Change', () => {
    ...
    it('ignores a late network refresh after the rendered stream changes', async () => {
      const firstStreamId = 'timeline:following:all' as PostStreamId;
      const secondStreamId = 'timeline:friends:all' as PostStreamId;
      vi.mocked(StreamPostsController.getOrFetchStreamSlice)
        .mockResolvedValueOnce({ nextPageIds: ['initial-x'], nextCursor: 10, reachedEnd: false })
        .mockResolvedValueOnce({ nextPageIds: ['current-y'], nextCursor: 20, reachedEnd: true });
      const { result, rerender } = renderHook(({ streamId }) => useStreamPagination({ streamId }), {
        initialProps: { streamId: firstStreamId },
      });
      await waitFor(() => expect(result.current.postIds).toEqual(['initial-x']));
      ...
      rerender({ streamId: secondStreamId });
      await waitFor(() => expect(result.current.postIds).toEqual(['current-y']));

      oldRefresh.resolve({ nextPageIds: ['obsolete-x'], nextCursor: 30, reachedEnd: true });
      await act(async () => {
        await oldRefreshPromise;
      });

      expect(result.current.postIds).toEqual(['current-y']);
      expect(result.current.loading).toBe(false);
    });
  ...
  describe('removePosts', () => {
    ...
    it('should keep a pending removal hidden when a network refresh replaces the page', async () => {
      const { result } = renderHook(() => useStreamPagination({ streamId: mockStreamId }));
      ...
      // A follow-driven refresh replaces the whole page from the network. The
      // backend has not seen the removal yet, so the replacement still carries
      // the row — it must stay hidden until the removal settles.
      vi.mocked(StreamPostsController.refreshStreamSlice).mockResolvedValueOnce({
        nextPageIds: mockPostIds,
        nextCursor: 42,
        reachedEnd: false,
    ...
    it('should re-apply a commit landing mid network refresh to the replacement cursor', async () => {
      const collectionStreamId = 'collection:author-1:post-1' as PostStreamId;
      vi.mocked(StreamPostsController.getCachedLastPostTimestamp).mockResolvedValue(0);
      vi.mocked(StreamPostsController.getOrFetchStreamSlice).mockResolvedValue({
        nextPageIds: ['c1', 'c2', 'c3'],
      ...
      let resolveRefresh: ((value: { nextPageIds: string[]; nextCursor: number }) => void) | undefined;
      vi.mocked(StreamPostsController.refreshStreamSlice).mockReturnValue(
        new Promise((resolve) => {
      ...
      await act(async () => {
        resolveRefresh?.({ nextPageIds: ['c1', 'c2', 'c3'], nextCursor: 3 });
        await refreshPromise;
      });
      ...
      // The replacement counted three raw rows against the pre-removal list, so
      // its absolute cursor has to absorb the decrement the commit applied
      ...
      // A committed removal releases its hidden entry, so a replacement page
      // fetched before the backend reindexed legitimately re-delivers the row —
      // the offset correction still has to hold for the pages that follow.

...
import { LocksContentController } from './locksContent';
...
vi.mock('@/application/locksContent/locksContent', () => ({
  LocksContentApplication: {
...
describe('LocksContentController', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    let minted = 0;
  ...
  it('uploads the attachments first, then the post built from their paths', async () => {
    ...
    const result = await LocksContentController.createLockContent({
  ...
  it('gives identical files distinct paths, so neither overwrites the other', async () => {
    await LocksContentController.createLockContent({
  ...
  it('builds the post with an empty list when there are no attachments', async () => {
    ...
    await LocksContentController.createLockContent({ buildPost: builder });
  ...
  it('sends the placeholder dev-static criterion and lock logic', async () => {
    await LocksContentController.createLockContent({ buildPost });
  ...
  it('rejects when there is no Locks session', async () => {
    ...
    const error = await LocksContentController.createLockContent({ buildPost }).catch((caught: unknown) => caught);
  ...
  it('does not build the post or create the lock when an attachment upload fails', async () => {
    ...
    await expect(
      LocksContentController.createLockContent({ attachments: [file('image/png')], buildPost: builder }),

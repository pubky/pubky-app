...
- **Icons**: stock Lucide from `lucide-react`; custom/brand SVG components from `@icons` (`src/libs/icons/icons.tsx` via `tsconfig` path alias). Do not re-export Lucide through libs barrels or import stock icons from a libs/icons barrel. URL→icon helpers (`getIconFromUrl`, `getLabelFromUrl`, …) live in `@libs/utils/urlToIcon` — see `docs/components.md` — _Icons (Lucide and custom)_.

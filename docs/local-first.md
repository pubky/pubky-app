...
- An anchor that left the row (its post deleted or un-bookmarked mid-scroll) is re-anchored on the deepest id the renderer has already shown (`visiblePostIds`, threaded down by `useStreamPagination`), and the walk restarts from the head when none is known. It never jumps to the row tail, which would skip every cached id in between. The bookmark mutators (`prependToStream`, `removeFromStream`) drop the cursor of a row they empty, so re-seeding it starts from the top; a row emptied by deleting its posts keeps it, which is safe because those posts are gone from Nexus too.

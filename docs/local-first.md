...
- A descending Nexus page is appended to the cached stream in stream order (`persistNewStreamChunk` with `tailCursor`), never re-sorted by `indexed_at`, so the raw anchor always sits at the tail. A page that adds no new id leaves the row untouched (at most recording a deeper cursor), and an unread merge sorts only the unread prefix it prepends. Cursor-less chunks that do add ids (hydration-discovered replies, ascending reply pages) keep sorting by timestamp.

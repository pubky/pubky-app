...
6. **Keep a signal for the persistent case.** A rule drops every instance that matches, including the ones that mean an outage. Either make the drop conditional on evidence the failure was expected (the `aborted-requests` rule only fires when `context.signalAborted` shows _our_ `AbortSignal` fired, set by `safeFetch`; browser-driven aborts stay reportable), or have the owner of the retry loop escalate once after a threshold. `MuteListSyncCoordinator` does the latter: routine `subscribeUserEventStreamForPath` failures are dropped, but after `MUTE_SYNC_STREAM_FAILURE_ALERT_THRESHOLD` consecutive failures it constructs one `Err.server(SERVICE_UNAVAILABLE)` tagged `muteListEventStreamExhausted`. Define "consecutive" carefully: a connection counts as healthy once it delivered an event _or_ stayed open for `MUTE_SYNC_STREAM_HEALTHY_AFTER_MS` (a healthy idle stream never completes a read), and every way a connection can end must apply that verdict — a throw, a clean close, and a controlled teardown (hide/show, route or auth change). A clean close before the connection proved healthy is a failed iteration, otherwise a homeserver that accepts and immediately closes every subscribe reconnects forever without reporting; a healthy connection torn down deliberately still clears the streak, otherwise earlier failures survive a hide/show cycle. The streak is scoped to the signed-in user so a second user's outage is neither hidden by nor inherits the first one's. That escalation must **not** pass the dropped error as `cause` — the once-per-chain guard would treat it as a wrapper and skip capture; put the last error's code/operation in `context` instead.

...
**Update, September 24, 2026:** the plugin and its two rules were removed (the other React Compiler rules are native in Oxlint) and the root-only `*.config.*` ignores were anchored, which adds `src/sentry.*.config.ts` back to linting. On the same machine and tree, whole-repository lint went from **7.32 s to 1.50 s** (three alternating runs each, CLI invoked through Node as described below) with identical diagnostics. The first CI run of that change took **4 s** for the lint step instead of 19 s ([code quality](https://github.com/pubky/pubky-app/actions/runs/35961995112)). The table above keeps the original September 17 figures.

...
npm test -- tooling/oxlint/pubky.test.ts   # Oxlint guardrail fixtures (part of npm test)
...
The Build workflow restores `.next/cache` separately from setup-node's npm download cache. Cache keys separate runner OS/architecture, dependencies and build configuration; every run restores the most recent compatible entry, but only pushes to `dev`/`master` save one (pull-request and merge-queue entries could not be restored by other refs and would only evict other caches). A missing cache simply causes a fresh build. Docker retains its existing layer caching; it does not transfer Next.js's incremental cache between fresh CI builders.

Production builds use Next.js's default Turbopack bundler, then `npm run build:sw` emits `public/sw.js` (see `docs/pwa.md`); never deploy the output of `next build` alone. `@serwist/cli` uses esbuild to bundle the worker; TypeScript 7 remains the sole type checker. Against the running server, the Build workflow fails when the precache does not cover the whole build or an entry is not served as built (`tooling/sw/verify-precache.mjs`). The Docker runner stage copies `public/` after the standalone tree so the freshly generated worker wins over a traced copy.
...
The project uses one compiler: `typescript@7.0.2`. Both `npm run typecheck` and Next.js production builds invoke its native `tsc` CLI. `next.config.ts` explicitly enables `experimental.useTypeScriptCli`; do not add a second TypeScript version to restore the old JavaScript compiler API. Cypress bundles specs with esbuild (`file:preprocessor` in `cypress/cypress.config.ts`) for the same reason: its default preprocessor runs ts-loader, which needs that API. Vitest uses Vite's built-in `resolve.tsconfigPaths` for aliases, avoiding the TypeScript 5 peer dependency in the old `vite-tsconfig-paths` / `tsconfck` integration.
...
Oxlint owns linting through `.oxlintrc.json`. Most rules are native, including the React Compiler rules. The existing import sorter, import padding rule, React deprecation rule and Next.js relative-navigation rule use JavaScript plugins to retain coverage. These plugins still install ESLint as a peer dependency; no script runs ESLint and no TypeScript ESLint parser is installed. Oxlint's JavaScript plugin API is currently alpha, so keep the plugins pinned and run the tooling fixtures when upgrading.

`tooling/oxlint/pubky.mjs` implements the existing selector-based restrictions. Their selectors, messages, override order and exceptions are preserved, including the test-file override replacing the environment selectors. `tooling/oxlint/pubky.test.ts` (part of `npm test`) checks prohibited forms, allowed forms, directory exceptions, root-only config exclusions, suppression comments and the private toast imports. Use `oxlint-disable` comments with the names from `.oxlintrc.json` and a reason. The one exception is the React hooks rules: suppress them as `// eslint-disable-next-line react-hooks/exhaustive-deps -- reason` (or `rules-of-hooks`). The React Compiler recognises only that spelling and then skips the component instead of compiling code whose dependencies were overridden; Oxlint honours it as an alias of `react/exhaustive-deps`.

Oxfmt owns formatting through `.oxfmtrc.json`, including Tailwind v4 class sorting with `src/app/globals.css` and `cn()`. Import sorting remains with the linter, and package.json sorting is disabled. Oxfmt 0.68 uses different union line wrapping from Prettier 3.9; the migration includes those formatting-only changes. Generated files and snapshots remain excluded.
...
- **No suppressed lints without a reason.** Every `oxlint-disable`, `oxlint-disable-next-line`, `eslint-disable-next-line react-hooks/*` (the only allowed eslint form, see _Compiler, Linter and Formatter_), `@ts-ignore` and `@ts-expect-error` carries an adjacent comment saying why it is needed and what would break without it. If a rule is wrong for a whole file, prefer one top-level `oxlint-disable` with rationale over inline ignores.

...
- **Updates are user-consented.** `skipWaiting: false`; the app registers the worker itself (`register: false`) after attaching lifecycle listeners, shows a persistent "Update available" toast on `waiting`, posts `SKIP_WAITING` on Reload, and reloads every tab once the new worker controls the page. `clientsClaim` stays on so the first install takes effect immediately.

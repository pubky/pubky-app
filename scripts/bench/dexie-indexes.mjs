...
function buildUserCountsRows(rows) {
  const out = [];
  for (let i = 0; i < rows; i++) {
    out.push({
      id: `pubky-user-${i}`,
...
function buildUserDetailsRows(rows) {
  const out = [];
  const now = Date.now();
  for (let i = 0; i < rows; i++) {
    out.push({
...
function buildUserRelationshipsRows(rows) {
  const out = [];
  for (let i = 0; i < rows; i++) {
    out.push({
      id: `pubky-user-${i}`,
...
function buildUserTtlRows(rows) {
  const out = [];
  const now = Date.now();
  for (let i = 0; i < rows; i++) {
    out.push({
...
function buildUserConnectionsRows(rows) {
  const out = [];
  for (let i = 0; i < rows; i++) {
    const following = [];
    const followers = [];
...
function buildUserConnectionsWorkloads({ table, rows, config }) {
  return [
    {
      key: 'bulkInsert',
      label: 'bulkInsert',
...
function buildUserCountsWorkloads({ table, rows, config }) {
  return [
    {
      key: 'bulkInsert',
      label: 'bulkInsert',
...
function buildUserDetailsWorkloads({ table, rows, config }) {
  return [
    {
      key: 'bulkInsert',
      label: 'bulkInsert',
...
function buildUserRelationshipsWorkloads({ table, rows, config }) {
  return [
    {
      key: 'bulkInsert',
      label: 'bulkInsert',
    ...
    },
    {
      ...
      run: async () => {
        for (let i = 0; i < config.lookups; i++) {
          const matches = await table.filter((row) => row.muted === (i % 2 === 0)).toArray();
          matches.slice(0, 100);
        }
      },
    },
...
function buildUserTtlWorkloads({ table, rows, config }) {
  return [
    {
      key: 'bulkInsert',
      label: 'bulkInsert',
...
const benchSuites = [
  ...
  },
  {
    ...
    baseline: {
      ...
      schema: normalizeSchema('&id, tagged, tags, unique_tags, posts, replies, following, followers, friends, bookmarks'),
      flags: {},
    },
    current: {
  ...
  },
  {
    ...
    baseline: {
      ...
      schema: normalizeSchema('&id, name, bio, image, indexed_at, links, status'),
      flags: {},
    },
    current: {
  ...
  },
  {
    ...
    baseline: {
      ...
      schema: normalizeSchema('&id, following, followed_by, muted'),
      flags: {},
    },
    current: {
